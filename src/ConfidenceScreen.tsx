...
const CONF_ALFRED_SRC = publicPath("/alfred/Confidence_Alf red.png".replace(" ", "_")); // tolerate space/underscore
...
function ConfidenceHelpContent() {
  return (
    <div style={{ display: "grid", gap: 12, lineHeight: 1.5 }}>
      <p>
        <em>Use this page for a one-minute reset before a meeting, interview, or whenever you want to feel steady and
          bold.</em>
      </p>
  ...
  return (
    <div className="card confidence-layout" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <svg viewBox="0 0 220 220" width={220} height={220} aria-label="Bloom breath pacer">
          ...
          <g transform="translate(110,110)">
            {[0, 1, 2, 3, 4, 5].map((i) => (
              <g
                key={i}
                transform={`rotate(${i * 60}) scale(${petalScale})`}
                style={{ transition: "transform 900ms ease-in-out", opacity: petalOpacity }}
              >
      ...
      <div style={{ display: "grid", gap: 10 }}>
        ...
        <div>
          <button onClick={() => onRep(patternKey)}>✓ Log breath rep</button>
        </div>
      </div>
    </div>
  );
...
function Affirmation({ running, onRep }: { running: boolean; onRep: (text: string) => void | Promise<void> }) {
  ...
  useEffect(() => {
    if (!running || !hasSet || items.length < 2) return;
    const id = setInterval(() => setIdx((i) => (i + 1) % items.length), 8000);
    return () => clearInterval(id);
  }, [running, hasSet, items.length]);
  ...
  useEffect(() => {
    if (!speak) return;
    const t = hasSet ? items[idx]?.text || "" : localStorage.getItem(FALLBACK_LS_KEY) || "";
    if (t) speakText(t);
    if (!running) return;
    const id = setInterval(() => t && speakText(t), 6000);
    return () => clearInterval(id);
  }, [speak, running, hasSet, idx, items]);
  ...
  return (
    <div className="card confidence-layout" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      <div style={{ display: "grid", gap: 12 }}>
        ...
        >
          {hasSet ? items[idx]?.text || "" : singleText}
        </div>

        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <label style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
            <input type="checkbox" checked={speak} onChange={(e) => setSpeak(e.target.checked)} />
            Speak while practicing
          </label>
          <div style={{ marginLeft: "auto", display: "inline-flex", gap: 6, alignItems: "center" }}>
            {hasSet && (
              <>
                <button onClick={() => setIdx((i) => (i - 1 + items.length) % items.length)} disabled={items.length <= 1}>
                  ◀ Prev
                </button>
                <span className="muted">
                  {idx + 1} / {items.length}
                </span>
                <button onClick={() => setIdx((i) => (i + 1) % items.length)} disabled={items.length <= 1}>
                  Next ▶
                </button>
              </>
            )}
        ...
        <div>
          <button onClick={() => onRep(hasSet ? items[idx]?.text || "" : singleText)}>✓ Log affirmation rep</button>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {hasSet ? (
          <>
            <div className="section-title">Today’s set</div>
              ...
              {items.map((a, i) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 8,
                    border: "1px solid",
                    borderColor: i === idx ? "hsl(var(--pastel-hsl))" : "#e5e7eb",
                    background: i === idx ? "hsl(var(--pastel-hsl) / .45)" : "#fff",
                    padding: "8px 10px",
                  }}
                >
            ...
            <div className="muted">
              Built in the Affirmation Builder. <a href="/affirmation-builder" className="btn-link">Open builder →</a>
            </div>
          </>
        ) : (
          <>
            ...
            {["I follow through on what matters today.", "I can handle this — one step at a time.", "I speak clearly and stay calm."].map(
              (s, idx2) => (
                <button key={idx2} onClick={() => setSingleText(s)} style={{ textAlign: "left" }}>
                  {s}
                </button>
              )
            )}
            <div className="muted">
              Want a set by category? <a href="/affirmation-builder" className="btn-link">Open builder →</a>
            </div>
          </>
        )}
      </div>
