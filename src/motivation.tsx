...
const STORAGE_KEY = "byb.motivationCentre.videos";
const MAX_VIDEOS = 10;
const WIN_SECONDS = 180; // copy of meditation mechanic (3 minutes)
...
function parseStartEnd(url: string): { start?: number; end?: number } {
  try {
    const u = new URL(url);
    const t = u.searchParams.get("t") || undefined; // can be 160 or 2m40s
    const startParam = u.searchParams.get("start") || undefined;
    const endParam = u.searchParams.get("end") || undefined;

    const toSecs = (val?: string) => {
      if (!val) return undefined;
      // supports "160", "160s", "2m40s", "2m", "1h2m3s"
      const m = val.match(
        /^(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s)?$|^(\d+)$/
      );
      if (!m) return Number(val);
    ...
    const start = toSecs(startParam ?? t);
    const end = toSecs(endParam);
...
export default function MotivationScreen() {
  ...
  useEffect(() => {
    ...
    const prepared: Video[] = SEEDS.map((s, i) => {
      ...
      const { start, end } = parseStartEnd(s.url || "");
      return {
        id,
        title: s.title || `Video ${i + 1}`,
        url: s.url || "",
        startSeconds: s.startSeconds ?? start,
        endSeconds: s.endSeconds ?? end,
      } as Video;
    });
    setVideos(prepared);
  }, []);
  ...
  function addVideo() {
    ...
    const { start, end } = parseStartEnd(link);
    setVideos((v) =>
      dedupeById([
        ...v,
        { id, title, url: link, startSeconds: start, endSeconds: end },
      ]).slice(0, MAX_VIDEOS)
    );
    setFormTitle("");
    setFormUrl("");
  ...
  function startTimer(id: string, title: string) {
    ...
    timersRef.current[id] = window.setInterval(() => {
      ...
      if (secs >= WIN_SECONDS && !completedRef.current.has(id)) {
        ...
        toast.show(`Motivation done — ${title}`);
      }
    }, 1000);
  }
    ...
    (async () => {
      ...
      const player = new YT.Player(host, {
        ...
        playerVars: {
          ...
          playsinline: 1,
          autoplay: 1,
          ...(video.startSeconds != null ? { start: video.startSeconds } : {}),
          ...(video.endSeconds != null ? { end: video.endSeconds } : {}),
        },
        events: {
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="https://youtu.be/… or https://www.youtube.com/watch?v=… (supports ?t=, &start=, &end=)"
                  className="mt-1 w-full border rounded-md px-3 py-2"
                />
              </div>
