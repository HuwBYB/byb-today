...
export default function TodayScreen({ externalDateISO }: { externalDateISO?: string }) {
  ...
  // If Calendar passes a date, use it
  useEffect(() => {
    if (externalDateISO) {
  ...
  // Load items + current affirmation for the chosen date
  async function refresh() {
    ...
    try {
      ...
      setAffirmationText(""); // keep input empty
    } catch (e: any) {
  ...
  useEffect(() => {
    if (userId) refresh();
  }, [userId, dateISO]); // re-run when date/user changes
  ...
  Day view — {dateISO}
      ...
      <div style={{ marginTop: 16, padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
        ...
        {loading ? (
          ...
          <div style={{ color: "#666" }}>No focus items yet.</div>
        ) : (
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#666" }}>
                    {i.item_type === "daily_action" ? "Big Goal step" : i.source}
                  </div>
                </div>
                <div>
                  <button onClick={() => setFocus(i.item_type, i.item_id, false)} style={{ marginRight: 8 }}>
                    Unfocus
                  </button>
      ...
      <div style={{ marginTop: 16, padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
        ...
        {loading ? (
          ...
          <div style={{ color: "#666" }}>Nothing else scheduled.</div>
        ) : (
          <ul style={{ listStyle: "none", padding: 0, margin: 0 }}>
            {others.map((i) => (
              <li
              ...
              >
                ...
                <div>
                  <button onClick={() => setFocus(i.item_type, i.item_id, true)} style={{ marginRight: 8 }}>
                    Make Top 3
                  </button>
                  <button onClick={() => completeItem(i.item_type, i.item_id)}>Done</button>
                </div>
      ...
      <div style={{ marginTop: 16, padding: 12, border: "1px solid #ddd", borderRadius: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          />
          <button onClick={addTask} style={{ padding: "8px 12px", border: "1px solid #333", borderRadius: 6 }}>
            Add
          </button>
        </div>
      </div>
